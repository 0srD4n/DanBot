...
anya({
    ...
    // Cek apakah username sudah diberikan
    if (args.length < 1) {
        return pika.reply(`*${Config.themeemoji} Example:* ${prefix + command} <username>\n\n> Bot will fetch details about the specified Instagram user.`);
    }

    const username = args[0]; // Ambil username dari argumen
    const { key } = await pika.keyMsg(Config.message.wait); // Pesan sementara

    try {
        // Request ke API untuk mendapatkan informasi pengguna Instagram
        const { data } = await axios.get(`https://www.guruapi.tech/api/igstalk?username=${username}`);
        
        // Format caption dari data yang didapat
        const caption = `
...
${data.biography}`;
        ...
        let pfp;
        try {
            // Mendapatkan profile picture pengguna, fallback ke default image jika gagal
            pfp = await getBuffer(data.profile_picture);
        } catch {
            pfp = await getBuffer(Config.imageUrl); // Default image jika gagal mendapatkan pfp
        }

        // Cek apakah ada UI khusus dengan tombol
        ...
        await pika.deleteMsg(key);
            ...
            console.error("Error fetching Instagram data:", err.response?.data || err.message || err);
